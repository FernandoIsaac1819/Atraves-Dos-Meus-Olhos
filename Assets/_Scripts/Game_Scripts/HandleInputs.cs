...
public class HandleInputs : MonoBehaviour
{
    ...
    public InputAction On_NextForm_Performed { get; private set; }
    ...
    public EventHandler OnTransform_Pressed;
    ...
    {
        OnTransform_Pressed?.Invoke(this, EventArgs.Empty);
    }
    ...
    //     m_CurrentMoveInput = Vector2.SmoothDamp(
    //         m_CurrentMoveInput, m_TargetMoveInput, ref m_SmoothInputVelocity, m_SmoothInputSpeed);

    //     // Return movement input in player-relative space
    //     Vector3 direction = new Vector3(m_CurrentMoveInput.x, 0, m_CurrentMoveInput.y);
    //     return direction.normalized; // No camera influence, purely raw input
    // }
    ...
    public Vector3 GetMovementDirection()
    {
        ...
        if (m_Cam != null)
        {
            // Convert movement direction based on the camera's rotation
